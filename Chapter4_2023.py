...
activities = pd.read_pickle(dirname + "20251222-activities.pkl")
regionpivot = pd.read_pickle(dirname + "20260103-final-regionpivot-GMM-unique.pkl")

regionpolys = pd.read_pickle(dirname + "20251218-fullallregionpolys.pkl")
...
# before running this, activities has length 
...
# drop the NaNs
# this is removing the activities that weren't taken in the random 20% slice

# after running it, activities has length 
...
regionsummary = activities[['Cardid', 'region_alpha', 'final_regionID', 
                            'region_type', 'Token_type', 'region_cluster']].drop_duplicates(
...
regioncount = pd.pivot_table(regionsummary, index=['Cardid', 'Token_type'], 
...
regionsummary1 = activities[['Cardid', 'region_alpha', 'final_regionID', 
                            'region_type', 'Token_type', 'region_cluster', 'region_name']].drop_duplicates(
...
regioncount1 = pd.pivot_table(regionsummary1, index=['Cardid', 'Token_type'], 
                             columns='region_name', aggfunc='count', 
                             values='final_regionID')
regioncount1 = regioncount1.fillna(0)
regioncount1.reset_index(inplace=True)

regionsummary2 = regioncount1[['Token_type','Education', 'Education/Leisure', 'Residences',
       'Residences/Leisure', 'Visited', 'Workplaces', 'Workplaces/Leisure']].value_counts()
...
tokensummary = pd.melt(
    regioncount1,
    id_vars=['Token_type'],
    value_vars=['Education', 'Education/Leisure', 'Residences',
           'Residences/Leisure', 'Visited', 'Workplaces', 'Workplaces/Leisure'],
)
...
totals_allocated = regioncount1['Token_type'].value_counts()
...
tokensummary = pd.merge(tokensummary, totals_allocated[['Token_type','token_total']], 
                        left_on='Token_type', right_on='Token_type',
                        how='left')
...
card_info = activities[['Cardid','Token_type','Start_h']]
card_tokens = card_info[['Cardid','Token_type']].drop_duplicates()
...
# 1.7% of cards have more than one token type in the month
...
cardsum = activities[['Day_h','Token_type','Period_h','Duration','final_regionID','Distance_travel',
                      'activity_alpha','region_alpha','Cardid','Travel_period_start']].groupby(
                          ['Cardid', 'Travel_period_start'], as_index=False).agg({'Token_type':'first','Day_h':list, 'Period_h':list,
                                                                                  'Duration':(list,'sum'),
                                                                                  'final_regionID':list,
...
#%% IDENTIFY CARDS WITH HOME REGIONS
...
print(len(homecounts), "cards have a home region") # 88,727
...
hubsinfo = pd.read_csv('C:\\Users\\megan\\Desktop\\PhD data\\hubs2017\\csv\\hubsinfoforRailSmart\HubsInfo_megan.csv', skiprows=3)
...
hub_coords = pd.read_csv('C:\\Users\\megan\\Desktop\\PhD data\\hubs2017\\csv\\hubsinfoforRailSmart\exportHubsStopTable.csv')
...
# this is the exact same underlying dataset as for the 2017 analysis
...
#%% FIND NEAREST HUB FUNCTIONS

def find_nearest_hub_OSM_graph(row):
    start = timeit.default_timer()
    mid_lat = row['geometry'].y
    mid_lon = row['geometry'].x
    
    osm = OSM('C:\\Users\\megan\\Desktop\\PhD data\\hubs2017\\csv\\perth_metro_downloaded.osm.pbf', 
              [mid_lon-4/111,mid_lat-4/111,mid_lon+4/111,mid_lat+4/111])
    ...
    print("graph read, " + str(timeit.default_timer() - start) + " seconds")
    ...
    print("candidate nodes done, " + str(timeit.default_timer() - start) + " seconds")
    ...
    for i in np.arange(0,5):
        ...
        if len(route[0])<=1:
            ...
            dist = gdf["length"].sum()
        dists = dists+[dist]
        print("this dist done, " + str(timeit.default_timer() - start) + " seconds")
    ...
    print("all dists done, " + str(timeit.default_timer() - start) + " seconds")
...
def get_route_dist(row, graph, i):
    if row['route_'+str(i)+'_len']<=1:
        return 0
    return ox.routing.route_to_gdf(graph, row['route_'+str(i)], weight='length')['length'].sum()

#%% DEMONSTRATE WHY OSM CHECK IS REQUIRED

# takes about 20-30s for one card
# card 1375432 is a random guess, not actually one that gets reallocated 
homeregion_test = homeregions[homeregions['Cardid']==1401039]
...
# this takes a few minutes but then we never have to do it again
osm = OSM('C:\\Users\\megan\\Desktop\\PhD data\\hubs2017\\csv\\perth_metro_downloaded.osm.pbf', [115.493774,-32.685620,116.180420,-31.447410])
nodes, edges = osm.get_network(nodes=True, network_type='walking')
G = osm.to_graph(nodes, edges, graph_type='networkx', osmnx_compatible=True)
...
start = timeit.default_timer()
home_unique['home_node'] = ox.nearest_nodes(G, X= home_unique['geometry'].x, Y=home_unique['geometry'].y)
print("home nodes done, " + str(timeit.default_timer() - start), " seconds")

for i in np.arange(0,5):
    home_unique = pd.merge(home_unique,hubs_def[['geometry']], left_on=str(i)+'_ix', right_index=True, how='left')
    if i==0:
        home_unique.rename(columns={'geometry_y':'candidate_'+str(i)}, inplace=True)
    else:
        home_unique.rename(columns={'geometry':'candidate_'+str(i)}, inplace=True)
    
    home_unique['candidate_node_'+str(i)] = ox.nearest_nodes(G, 
                                                    X=home_unique['candidate_'+str(i)].x, 
                                                    Y=home_unique['candidate_'+str(i)].y)

    home_unique['route_'+str(i)] = ox.shortest_path(G, 
                                                    home_unique['home_node'], 
                                                    home_unique['candidate_node_'+str(i)], 
                                                    weight="length")
    home_unique['route_'+str(i)+'_len'] = home_unique['route_'+str(i)].str.len()
    home_unique['dist_'+str(i)] = home_unique.apply(lambda row: get_route_dist(row, G, i), axis=1)
    print("distance "+str(i)+" done, " + str(timeit.default_timer() - start), " seconds")
    
home_unique['ix_min'] = home_unique[['dist_0','dist_1','dist_2','dist_3','dist_4']].idxmin(axis=1)
home_unique['ix_min'] = home_unique['ix_min'].str[-1]+'_ix'

col_indexer = home_unique.columns.get_indexer(home_unique['ix_min'])
home_unique['hubs_loc'] = home_unique.values[range(len(home_unique)), col_indexer]

home_unique = pd.merge(home_unique, hubs_def[['hubID']], left_on='hubs_loc', right_index=True, how='left')
home_unique.rename(columns={'hubID':'nearest_hub_osm'}, inplace=True)

home_unique.drop(columns={'home_node','candidate_node_0', 'route_0',
'route_0_len', 'dist_0', 'candidate_1', 'candidate_node_1', 'route_1',
'route_1_len', 'dist_1', 'candidate_2', 'candidate_node_2', 'route_2',
'route_2_len', 'dist_2', 'candidate_3', 'candidate_node_3', 'route_3',
'route_3_len', 'dist_3', 'candidate_4', 'candidate_node_4', 'route_4',
'route_4_len', 'dist_4', 'ix_min', 'candidate_0','hubs_loc'}, inplace=True)
home_unique.rename(columns={'geometry_x':'geometry'}, inplace=True)
print("all done, " + str(timeit.default_timer() - start), " seconds")

home_unique.to_pickle(dirname+'20260301-home_unique_all.pkl')


#%% CALCULATE NUMBER THAT HAVE NEW GEOMETRIES

len(home_unique[home_unique['nearest_hub']!=
                   home_unique['nearest_hub_osm']])/len(home_unique)*100
# 12.6% of geometries have a different nearest hub when using the street network approach

#%% MERGE BACK ONTO FULL DATASET
# merge on just the geometry, due to the duplicate geometries (geometry is the unique feature)

homeregions = pd.merge(homeregions, home_unique[['geometry','nearest_hub_osm']], 
...
cardsum.to_pickle(dirname + "20260302-cardsum.pkl")
activities.to_pickle(dirname + "20260302-processed_activities.pkl")

#%% READ FILES

cardsum = pd.read_pickle(dirname + "20260302-cardsum.pkl")
...
# activities['final_regionID'].unique()
mapping_AV = {'L2':'A', 
              'V0.0':'V', 
              'L1':'A', 
              'W1':'A', 
...
G = nx.from_dict_of_dicts(allgraphs_unique[11], create_using=nx.DiGraph())
H = nx.from_dict_of_dicts(allgraphs_unique[113], create_using=nx.DiGraph()) 
...
print("edit distance, with node labels: ", nx.graph_edit_distance(G, H, node_match=nm)) # 1
...
print("edit distance, with node labels and weights: ", nx.graph_edit_distance(G, H, node_match=nm, edge_match=em)) # 3
...
distances.to_pickle(dirname + "20260103-distances.pkl")

#%% OPTION 2: READ DISTANCE FOR RANDOM SAMPLE OF CARDS

distances = pd.read_pickle(dirname + "20260103-distances.pkl")
...
test = pd.merge(test, test_unique[['region list', 'iso_group', 'iso_group labelsAV']], # 'iso_group labels','iso_group labelsV', 
                left_on='region list', right_on='region list', how='left')

#%% PLOT AN EXAMPLE

group = 1 # groups are numbered sequentially, the group number doesn't refer to the index
...
# len(test[test['diameter_dir'].isna()])/len(test) = approx 25.5%
...
test.rename(columns={'Token_type first':'Token_type'}, inplace=True)

test.to_pickle(dirname + "20260302-testwithclusters.pkl")

#%% OPTION: READ PICKLE OF FINAL DATASET WITH CLUSTERS

test = pd.read_pickle(dirname + "20260302-testwithclusters.pkl") 
...
this_figure = sp.make_subplots(rows=6, cols=6, vertical_spacing = 0.09)
...
def make_heatmap(df, yval, zval):
    fig = px.density_heatmap(df, x=colname_init, 
                             nbinsx=4,y=yval,
                             z=zval,histfunc="avg",
                             color_continuous_scale=["#EFEFEF",mycolors_discrete[1]],
                             category_orders={'variable':['Morning','Day','Afternoon','Night']},
                             text_auto=".2f",
                             labels={
                                 "value": " ",
                                 "variable": " ",
                                 colname_init: "Cluster",
                             },
                         )
    fig.update_layout(default_layout)
    fig.update(layout_coloraxis_showscale=False)
...
fig = make_heatmap(weekendsummary, "variable", "lift")

#fig = make_heatmap(timesummary, "variable", "lift")

#fig = make_heatmap(tokensummary, "value","lift")

pyo.plot(fig, config=config)

# filename = "routines_lifttable_weekend"
# fig.write_html(dirname+filename+".html", include_plotlyjs='cdn', config=config)
# fig.write_image(dirname+filename+".svg")
...
fig = px.scatter_map(hub_centre_point, 
                        ...
                        color=hub_centre_point['hubID'].astype(str), 
                        color_discrete_sequence=mycolors_discrete, 
                        size_max=15, zoom=10)
fig.update_layout(map_style="light")
pyo.plot(fig, config=config)
