...
journeys = pd.read_csv(
    dirname + "patrec_monthly_data_Aug-2023.csv",
    usecols=[
        "Cardid",
        "OnDate",
        "OnLocation",
...
print(TOTAL_LEN, "trips in original dataset")  # 10,298,921
v = journeys["Cardid"].value_counts()
...
token_map = {'Standard':'Standard',
            'Student 50 cent':'School',
             ...
             'Veteran':'Concession',
             'Convention Pass':'Concession'}

journeys["Token_type"] = journeys["Token"].map(token_map)

#%% PICKLE JOURNEYS DATA

journeys.to_pickle(dirname + "20251218-processed_journeys.pkl")
...
clusters = pd.read_pickle(dirname + "20251218-clusters.pkl")
geo = pd.read_pickle(dirname + "20251218-geo.pkl")

#%% STOP DATA

busStops = pd.read_csv(dirname + "ptawa_busStop_Extract_Aug-2023.csv")
trainStops = pd.read_csv(dirname + "ptawa_trainStation_Extract_Aug-2023_modified.csv")
...
clusters.to_pickle(dirname + "20251218-clusters.pkl")
geo.to_pickle(dirname + "20251218-geo.pkl")
...
activities = pd.melt(
    journeys,
    id_vars=["Cardid", "Token_type", "OnLocation", "OffLocation"],
...
activities["Token_type"], activities["class"] = learn_stay_class(
    activities, "Duration", "Start_h_only", "Token_type", "Distance_stay")
...
# drop activities where we don't know where they are
activities = activities.drop(activities[(activities['location']==0)].index).reset_index(drop=True)
...
activities.to_pickle(dirname + "20251222-activities.pkl")
...
cards = pd.read_pickle(dirname + "20251222-cards.pkl")
...
#%%
cards.to_pickle(dirname + "20251222-cards.pkl")

#%% LAND USE DATA

# (coords have already been updated)
bbox = (114.493774,-33.685620,117.180420,-30.447410) # more generous to 
# ensure entire area is covered

# check it has been set up correctly
geometry = [box(114.493774,-33.685620,117.180420,-30.447410)] 
...
mbpoly = gpd.read_file(dirname + "MB_2021_AUST_GDA2020.shp", bbox=bbox)
...
mbpoly["MB_CAT21"] = mbpoly["MB_CAT21"].map(landmap)

mbdissolve = mbpoly.dissolve(by="MB_CAT21")
mbdissolve.reset_index(inplace=True)

#%% PLOT LAND USE DATA TO SENSE CHECK

fig = px.choropleth_map(
    mbdissolve,
    geojson=mbdissolve.geometry,
    locations=mbdissolve.index,
    color="MB_CAT21",
    center={"lat": -31.95, "lon": 115.85},
    color_continuous_scale=mycolors_discrete,
    opacity=0.5,
...
allregions = pd.read_pickle(dirname + "20251218-fullallregions.pkl") 
...
for index, row in cardrun.iterrows():
    ...
    hist["Card"] = row["Cardid"]
    region_summary["Card"] = row["Cardid"]
    regionpolys["Card"] = row["Cardid"]
...
allhist.to_pickle(dirname + "20251218-fullallhist.pkl")
allregions.to_pickle(dirname + "20251218-fullallregions.pkl")
allregionpolys.to_pickle(dirname + "20251218-fullallregionpolys.pkl")
...
filename = "regions_numvisitshist_lt20_colour"
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=700, height=350)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
landusepolys = pd.read_pickle(dirname + "20251222-final-landusepolys.pkl")
regionpivot = pd.read_pickle(dirname + "20251222-final-regionpivot.pkl")
...
landusepolys.to_pickle(dirname + "20251222-final-landusepolys.pkl") 
...
allregions.to_pickle(dirname + "20251222-final-allregions.pkl") 
regionpivot.to_pickle(dirname + "20251222-final-regionpivot.pkl") 
...
colname = "GMM_cluster"
print(len(X), "records used in clustering (full dataset)")
...
print(regionpivot[colname].value_counts())

#%% SAVING FINAL OUTPUT DATA BEFORE PLOTTING

regionpivot.to_pickle(dirname + "20251222-final-regionpivot-GMM-full.pkl")
outland.to_pickle(dirname + "20251222-outland.pkl")
outregion.to_pickle(dirname + "20251222-outregion.pkl")

#%% OPTION TO READ DATA

regionpivot = pd.read_pickle(dirname + "20251222-final-regionpivot-GMM-full.pkl")
outland = pd.read_pickle(dirname + "20251222-outland.pkl")
outregion = pd.read_pickle(dirname + "20251222-outregion.pkl")
...
for i in np.arange(0,outland[colname].max()):
        fig.add_shape(
            type="line", xref='x', yref='y2 domain',
                            x0=i+0.5, y0=0, x1=i+0.5, y1=1, line_color=line_colour, line_width=line_width)
...
print(len(X), "records used in clustering (unique dataset)")
...
X[colname_unique] = GMM.predict(X)

print(X[colname_unique].value_counts())

print(regionpivot[colname_unique].value_counts())

#%% SAVING FINAL OUTPUT DATA BEFORE PLOTTING

regionpivot.to_pickle(dirname + "20260103-final-regionpivot-GMM-unique.pkl")
outland_unique.to_pickle(dirname + "20260103-outland_unique.pkl")
outregion_unique.to_pickle(dirname + "20260103-outregion_unique.pkl")

#%% OPTION TO READ DATA

regionpivot = pd.read_pickle(dirname + "20260103-final-regionpivot-GMM-unique.pkl")
outland_unique = pd.read_pickle(dirname + "20260103-outland_unique.pkl")
outregion_unique = pd.read_pickle(dirname + "20260103-outregion_unique.pkl")
...
#     2: "Residences",
#     3: "Workplaces",
#     4: "Residences/Leisure",
#     5: "Workplaces/Leisure"
# }
...
filename = "regions_activityboxplot_unique"
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=1800, height=700)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
filename = "regions_landuseboxplot_unique"
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=1800, height=700)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
instability = {"num_clusters": [n for n in np.arange(2,k_max)], 
               "10%": [val10[i-2] for i in np.arange(2,k_max)],
               "20%": [val20[i-2] for i in np.arange(2,k_max)],
               "30%": [val30[i-2] for i in np.arange(2,k_max)],
               "40%": [val40[i-2] for i in np.arange(2,k_max)],
               "50%": [val50[i-2] for i in np.arange(2,k_max)],
               "60%": [val60[i-2] for i in np.arange(2,k_max)]}
...
len(cardsum[(cardsum['Workplaces']==1)|(cardsum['Workplaces/Leisure']==1)])/len(cardsum)*100

# of those that have any workplace regions, how many only have one
len(cardsum[(cardsum['Workplaces']==1)|(cardsum['Workplaces/Leisure']==1)])/len(cardsum[(cardsum['Workplaces']>0)|(cardsum['Workplaces/Leisure']>0)])*100
...
test = test[test['region_cluster']==8]
...
ABSoverlay.rename(columns={'MB_CAT21':'Land use'}, inplace=True)
