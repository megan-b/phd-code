...
dirname = "C:\\Users\\megan\\Desktop\\PhD data\\hubs2017\\csv\\aug2023\\"
...
activities = pd.read_pickle(dirname+"20260302-processed_activities.pkl")
...
# there are 207 different combinations of stay counts by activity
...
# there are 152 different breakdowns of activity profiles
...
journeys = pd.read_pickle(dirname + "20251218-processed_journeys.pkl")
...
busStops = pd.read_csv(dirname + "ptawa_busStop_Extract_Aug-2024.csv")
trainStops = pd.read_csv(dirname + "ptawa_trainStation_Extract_Aug-2024_modified.csv")
...
clusters_old = pd.read_pickle(dirname + "20251218-clusters.pkl")
geo_old = pd.read_pickle(dirname + "20251218-geo.pkl")
...
geo['newName'] = geo['newName'].str.split(' Cat Id').str[0]
...
clusters.to_pickle(dirname + "20260331-clusters.pkl")
geo.to_pickle(dirname + "20260331-geo.pkl")

#%% ALTERNATIVE: READ STOP CLUSTERS INSTEAD

geo = pd.read_pickle(dirname + "20260331-geo.pkl")
clusters = pd.read_pickle(dirname + "20260331-clusters.pkl")
...
# this is all of the regions - both visited and anchor
allhist = pd.read_pickle(dirname + "20251218-fullallhist.pkl")
...
journeys['OnLocation_regioncluster'] = np.where((journeys['OnTran'].str.contains("Transfer"))|
                                                      (journeys['OnLocation_regioncluster'].isna())&(journeys['Cardid'].isin(allhist_merge['Card'])),
...
journeys['OffLocation_regioncluster'] = np.where((journeys['OffTran'].str.contains("Transfer"))|
                                                      (journeys['OffLocation_regioncluster'].isna())&(journeys['Cardid'].isin(allhist_merge['Card'])),
...
journeys_final.to_pickle(dirname+'20260305-journeys_final.pkl')
# 205,886 rows

#%% PLOT EXPLORING WHICH DURATION TO KEEP

# stop 2776 = Perth = spatial cluster 3
# stop 2764 = Subiaco = spatial cluster 2920

example = journeys_short[(journeys_short['OnLocation_cluster']==3)&(journeys_short['OffLocation_cluster']==2920)]
...
pyo.plot(fig)
# 80% of edges have duration ~24min or less


#%% READ JOURNEYS FINAL PICKLE 

journeys_final = pd.read_pickle(dirname+'20260305-journeys_final.pkl')
...
allregions = pd.read_pickle(dirname + "20251222-final-allregions.pkl")
...
allregionpolys = pd.read_pickle(dirname + "20251218-fullallregionpolys.pkl")
...
# this is now 179,350 in the 2017 data
# and 199,562 in the 2023 data
...
geo['spatial_cluster_old'] = geo['spatial_cluster_old'].astype(float)
allregion_list['spatial_cluster'] = allregion_list['spatial_cluster'].astype(float)

# this is the old definitions of spatial clusters remember
allregion_list = pd.merge(allregion_list, geo[['spatial_cluster','spatial_cluster_old']], left_on='spatial_cluster', right_on='spatial_cluster_old', how='left')

allregion_list.drop(columns=['spatial_cluster_old','spatial_cluster_x'], inplace=True)
allregion_list.rename(columns={'spatial_cluster_y':'spatial_cluster'}, inplace=True)

allregion_list = allregion_list[['Card','region_cluster','spatial_cluster']].drop_duplicates().reset_index(drop=True)
...
uniques.to_pickle(dirname+"20260305-uniques.pkl")
allregion_list.to_pickle(dirname+"20260305-allregionlist.pkl")
...
journeys_V.to_pickle(dirname+'20260305-journeys-V.pkl')

#%% READ JOURNEYS_V AND UNIQUES - CAN START HERE

journeys_V = pd.read_pickle(dirname+'20260305-journeys-V.pkl')
# this is 182,374

uniques = pd.read_pickle(dirname+"20260305-uniques.pkl")
# this is 41,843 records
...
geometry = [box(115.4,-31.939116,116.5,-31.4)] 
...
fig = px.scatter_map(
    ...
    color=clusters['NOR check']&clusters['bbox check'], # change here to run for other checks
    color_discrete_sequence=mycolors_discrete,
    center=map_center,
    zoom=12,
)
...
# this has 5385 records in it - there are only 6797 in the whole dataset
...
# only 120 locations contribute >0.1% of the records, which gets us to 64% coverage

used_V[used_V['percent']>1]
# only 15 contribute >1%, which is 27% coverage

len(used_V[used_V['cum_percent']>95])
# 2842 locations collectively contribute 5% of the visited activities
...
len(df_nodes) # 6797
len(df_nodes[df_nodes['NOR check']==True]) # 2865
len(df_nodes[df_nodes['SOR check']==True]) # 3543
len(df_nodes[df_nodes['outside CBD check']==True]) # 6748
len(df_nodes[df_nodes['jdlp check']==True]) # 919
len(df_nodes[df_nodes['train check']==True]) # 74
...
len(df_nodes[(df_nodes['is_open']==True)]) # 5383
len(df_nodes[(df_nodes['NOR check']==True)&(df_nodes['is_open']==True)]) # 2251
len(df_nodes[(df_nodes['SOR check']==True)&(df_nodes['is_open']==True)]) # 2790
len(df_nodes[(df_nodes['outside CBD check']==True)&(df_nodes['is_open']==True)]) # 5338
len(df_nodes[(df_nodes['jdlp check']==True)&(df_nodes['is_open']==True)]) # 775
len(df_nodes[(df_nodes['train check']==True)&(df_nodes['is_open']==True)]) # 74
# so all of them for trains - this isn't particularly useful
...
df_nodes.to_pickle(dirname+"20260322-df-nodes.pkl")
...
uniques.reset_index(inplace=True)
# length of uniques = 41843
# length of uniques in Perth metro (bbox=true) = 39711
...
allregion_list = pd.read_pickle(dirname+"20260305-allregionlist.pkl")
# this is regionIDs for cards and region clusters
...
# this was 51,322 in 2017 with only the journeys that have home locations being considered
# and 60,038 in 2023

journeys_cards.to_pickle(dirname+'20260322-journeys-cards.pkl')
# this is 88,261 in 2023, was 83,613 in 2017
...
# this was 54,019 in 2017- not a huge drop as we chopped the earlier analysis
...
len(journeys_unique[journeys_unique['NOR check']==True]) # 12504
len(journeys_unique[journeys_unique['SOR check']==True]) # 20169
len(journeys_unique[journeys_unique['outside CBD check']==True]) # 44509
len(journeys_unique[journeys_unique['jdlp check']==True]) # 3729
...
# merge on the checks
uniques_full = pd.merge(uniques_full, uniques, left_index=True, right_index=True, how='left')

uniques_plot = uniques_full
uniques_plot['ID_x'] = uniques_plot['ID_x'].astype(str)
...
ID_interest = 48
journeys_interest['Cardid'][journeys_interest['OffLocation_ID']==ID_interest].value_counts()

#%% SENSE CHECKING - INDIVIDUAL CARD

card_interest = 34806374 
...
distances.to_pickle(dirname + "20260302-distmatrix.pkl")
...
distances = pd.read_pickle(dirname + "20260302-distmatrix.pkl")
# this is a df where the distance is the distance FROM the
# first column TO the cluster along the columns of the first row
...
distances_to.to_pickle(dirname+"20260322-distances_to.pkl")
distances_from.to_pickle(dirname+"20260322-distances_from.pkl")
...
distances_to = pd.read_pickle(dirname+"20260322-distances_to.pkl")
distances_from = pd.read_pickle(dirname+"20260322-distances_from.pkl")
...
journeys_unique.to_pickle(dirname+"20260322-journeys-unique.pkl")
journeys_agg_all.to_pickle(dirname+"20260322-journeys-agg-all.pkl")

#%% READ PICKLES

journeys_unique = pd.read_pickle(dirname+"20260322-journeys-unique.pkl")
journeys_agg_all = pd.read_pickle(dirname+"20260322-journeys-agg-all.pkl")
...
# GET THE FIRST ON LOCATION FOR REFERENCE - use this as the location for the 'standard' p median
...
journeys_distances.to_pickle(dirname+"20260322-journeys-distances.pkl")
...
journeys_distances_trad.to_pickle(dirname+"20260322-journeys-distances-trad.pkl")


#%% DISTANCES FOR USING 'HOME' LOCATION

journeys_cards = pd.read_pickle(dirname+'20260322-journeys-cards.pkl')
# we have 88,261 of these with a home ID

journeys_cards['Cardid'].value_counts()
# there are 47,635 cards

journeys_cards['homeID'].value_counts()
# and there are 17,382 different regions used as home regions
...
journeys_distances_home.to_pickle(dirname+"20260322-journeys-distances-home.pkl")
...
def count_na_dist(row):
    return journeys_distances[row['clusterID']].isna().sum()
...
df_nodes = df_nodes[df_nodes['clusterID']!=8428]

df_nodes.to_pickle(dirname+"20260322-df-nodes.pkl")

#%% READ JOURNEYS_DISTANCES AND DF_NODES

journeys_distances = pd.read_pickle(dirname+"20260322-journeys-distances.pkl")
journeys_distances_trad = pd.read_pickle(dirname+"20260322-journeys-distances-trad.pkl")
journeys_distances_home = pd.read_pickle(dirname+"20260322-journeys-distances-home.pkl")

df_nodes = pd.read_pickle(dirname+"20260322-df-nodes.pkl")
...
def create_inputs(df_nodes, journeys_distances_home, journeys_distances, journeys_distances_trad, location_str, save_str):
    ...
    journeys_input.to_pickle(dirname+str(max_size)+"-journeys-input-"+save_str+"-home-2023.pkl")
    ...
    journeys_input = journeys_input.loc[:, journeys_input.columns.isin(potential_facilities)]     
    journeys_input.to_pickle(dirname+str(max_size)+"-journeys-input-"+save_str+"-mine-2023.pkl")
    ...
    journeys_input = journeys_input.multiply(journeys_input["count"], axis="index")
    journeys_input = journeys_input.loc[:, journeys_input.columns.isin(potential_facilities)]
    journeys_input.to_pickle(dirname+str(max_size)+"-journeys-input-"+save_str+"-trad-2023.pkl")

def run_algorithm(filename_suffix, data_suffix, run_type, pvals, max_rows):
    journeys_input = pd.read_pickle(dirname+str(max_rows)+"-journeys-input-"+filename_suffix+"-"+run_type+"-2023.pkl")
    ...
    for i in np.arange(0,len(pvals)):
        ...
        assignments.to_pickle(dirname+str(max_rows)+"-assignments-"+filename_suffix+"-"+str(pvals[i])+"-"+run_type+"-2023.pkl")
...
p=10
assignments = pd.read_pickle(dirname+"pmed results\\7200000-assignments-"+filename_suffix+"-"+str(p)+"-mine-2023.pkl")
assignments_trad = pd.read_pickle(dirname+"pmed results\\7200000-assignments-"+filename_suffix+"-"+str(p)+"-trad-2023.pkl")
assignments_home = pd.read_pickle(dirname+"pmed results\\7200000-assignments-"+filename_suffix+"-"+str(p)+"-home-2023.pkl")
...
for p_explore in [10, 20, 30]:
    ...
    #pyo.plot(fig, config=config)
    filename = "pmed_timesaved_dist_"+str(p_explore)
    fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
    fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=700, height=350)
    fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
assigned_facilities_all.to_csv(dirname+"assigned_facilities.csv")
df_nodes_output_all[['clusterID','is_open','pmed_open','count','newName','p']].to_csv(dirname+"df_nodes_output.csv")
journeys_agg_card_all.to_csv(dirname+"journeys_agg_card_all.csv")
...
pyo.plot(fig, config=config)
#%%
filename = "pmed_scatter_transport"
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=700, height=350)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
filename = "pmed_jdlp_allocated_map_"+str(p_explore)
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=700, height=350)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
filename = "pmed_jdlp_totalsaved_map_"+str(p_explore)
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=700, height=350)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)
...
# TODO this doesn't share scaling,
# is therea better way to do subplots or specify size
p_explore = 30
assigned_facilities = combined_agg_dist[(combined_agg_dist['time_saved']>0)&(combined_agg_dist['p']==p_explore)].groupby(['assignments']).agg({'time_saved':['sum','count','min', 'median', 'mean','max']})
df_nodes_output = combined_df_nodes_output[combined_df_nodes_output['p']==p_explore]
...
filename = "pmed_jdlp_mediansaved_map_"+str(p_explore)
fig.write_html(dirname+"figs\\html\\"+filename+".html", include_plotlyjs='cdn', config=config)
fig.write_image(dirname+"figs\\svg\\"+filename+".svg", width=700, height=350)
fig.write_image(dirname+"figs\\square\\"+filename+".svg", width=600,height=600)